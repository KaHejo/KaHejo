...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-400 text-xs sm:text-sm mt-1">Buat lencana pencapaian baru untuk memotivasi aksi dekarbonisasi pengguna.</p>
        </div>
        ...
        <form action="{{ route('admin.achievements.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       name="name" 
                       value="{{ old('name') }}" 
                       placeholder="Misal: Penjaga Listrik Bijak / Pelopor Zero-Waste" 
                       required 
                       class="admin-input">
                @error('name')
            ...
            <div>
                ...
                <textarea id="description" 
                          name="description" 
                          rows="3" 
                          placeholder="Jelaskan tindakan yang harus diselesaikan pengguna untuk membuka lencana ini..." 
                          required 
            ...
            <div>
                ...
                <select name="category" id="category" required class="admin-input">
                    <option value="" disabled {{ old('category') ? '' : 'selected' }} class="bg-[#0b1c15]">Pilih kategori emisi</option>
                    <option value="electricity" {{ old('category') == 'electricity' ? 'selected' : '' }} class="bg-[#0b1c15]">Listrik (Electricity)</option>
                    <option value="gasoline" {{ old('category') == 'gasoline' ? 'selected' : '' }} class="bg-[#0b1c15]">Bensin (Gasoline)</option>
                    <option value="diesel" {{ old('category') == 'diesel' ? 'selected' : '' }} class="bg-[#0b1c15]">Solar (Diesel)</option>
                    <option value="gas" {{ old('category') == 'gas' ? 'selected' : '' }} class="bg-[#0b1c15]">Gas Alam (Natural Gas)</option>
                    <option value="lpg" {{ old('category') == 'lpg' ? 'selected' : '' }} class="bg-[#0b1c15]">LPG</option>
                </select>
                @error('category')
                    <p class="text-xs text-rose-400 mt-1 flex items-center gap-1">
                        <i class="fa-solid fa-circle-exclamation text-[10px]"></i>
            ...
            <div>
                ...
                <div class="p-4 rounded-xl border border-dashed border-emeraldBrand/30 bg-emerald-950/20 text-center cursor-pointer hover:bg-emerald-950/30 transition-colors">
                    ...
                    <p class="text-[11px] text-slate-400 mt-2">Mendukung format PNG, JPG, SVG, WEBP (Transparan disarankan).</p>
                </div>
                @error('icon')
                    <p class="text-xs text-rose-400 mt-1 flex items-center gap-1">
                        <i class="fa-solid fa-circle-exclamation text-[10px]"></i>
